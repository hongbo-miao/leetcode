...
class TimeMap {
  ...
  get(key, timestamp) {
    ...
    const idx = this.lowerBound(arr, timestamp);
    if (idx < arr.length && arr[idx].timestamp === timestamp) {
      return arr[idx].value;
    } else if (idx - 1 >= 0 && arr[idx - 1].timestamp < timestamp) {
      return arr[idx - 1].value;
    } else{
      return '';
    }
  }
